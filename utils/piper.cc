  ...
  vector<string> args)
{
  ...
  if (pid == 0)
  {
    ...
    // Don't redirect stderr, otoherwise we don't know if something goes wrong
...
string piper_t::readline() {
  string msg;
  while(true) {
    uint64_t n = msg.size();
    msg.resize(n + 1);
    char* raw = msg.data() + n;
    auto r = ::read(inpipefd[0], raw, 1);
    if(r != 1) {
      throw std::runtime_error("could not read!");
    }
    if(msg.back() == '\n') {
...
void piper_t::writeline(string const& msg) {
  write(msg);
  write("\n");
}
